"""Trains XGBoost, GraphSAGE, and HGT on the AML synthetic dataset and
reports a comparison table -- the "prove the platform" checkpoint for the
generalized graph schema (`data/graph/domain_schema.py`,
`data/graph/build_domain_graph.py`). `XGBoostFraudModel`, `GraphSAGEFraudModel`,
and `HGTFraudModel` are reused completely unmodified from the payments
pipeline -- if this script produces a coherent result, that's direct
evidence the model layer really was domain-agnostic, not just claimed to be.
...
from ..data.graph.build_domain_graph import build_domain_hetero_graph
...
from ..domains.aml.aml_adapter import AMLAdapter
...
def main() -> None:
    ...
    parser.add_argument("--report-dir", default="artifacts/aml")
    ...
    # Homogeneous transaction-projection graph -- unmodified from payments,
    # works unchanged because AML's canonical column names (sender_id,
    # device_id, ip_address) match payments' by design.
    ...
    # Heterogeneous graph via the NEW generalized builder -- this is the
    # actual platform proof point.
