...
import { ArrowLeft, Check, XCircle, HelpCircle, X, Loader2, User, Building2 } from 'lucide-react';
...
import {
  ...
  import {   AlertTriangle, ChevronRight } from 'lucide-react'
    ...
    const handleVerifyAccount = async () => {
        if (recipientAccountNumber.length !== 10 || !recipientBankCode) {
        ...
        try {
          ...
          setResolvedAccountName(resolveData.account_name);
          setRecipientUpiId(`${recipientAccountNumber}@${recipientBankCode}`);

          // Step 2: Run AI fraud check using model data from Firestore
          const usersRef = collection(db, "users");
          const q = query(usersRef, where("upiId", "==", `${recipientAccountNumber}@${recipientBankCode}`));
          const querySnapshot = await getDocs(q);
          ...
          if (!querySnapshot.empty) {
            ...
            setVerificationStatus(predResult.prediction[0] === 1 ? "fraud" : "valid");
          } else {
            // Account resolved from Paystack but not in our DB — run fraud model with default features
            const defaultFeatures = [
              parseFloat(amount) || 0,  // Transaction Amount
              1,                          // Transaction Frequency (low for new user)
              0,                          // Recipient Blacklist Status
              0,                          // Device Fingerprinting
              0,                          // VPN or Proxy Usage
              0.5,                        // Behavioral Biometrics (unknown)
              24,                         // Time Since Last Transaction
              50,                         // Social Trust Score (mid for unknown)
              0.1,                        // Account Age (new)
              0,                          // High-Risk Transaction Times
              0,                          // Past Fraudulent Behavior Flags
              0,                          // Location-Inconsistent Transactions
              parseFloat(amount) ? Math.min(parseFloat(amount) / 10000, 1) : 0,  // Normalized Transaction Amount
              0.5,                        // Transaction Context Anomalies
              ...
              1,                          // Recipient Verification Status_verified
    ...
    return (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="min-h-screen" style={{background:'var(--background)',color:'var(--foreground)'}}
        >
          {!user ? (
            <motion.div
            ...
            >
              <div className="font-mono text-[10px] uppercase tracking-widest opacity-40">Shieldpay / AI.FRAUD</div>
              <h1 className="font-display italic font-black text-5xl tracking-tighter">SHIELD<span style={{color:'var(--accent)'}}>PAY.</span></h1>
              <button
                onClick={handleGoogleSignIn}
                style={{backgroundColor:'var(--accent)'}}
                className="px-6 py-3 text-white font-mono text-xs uppercase tracking-widest hover:brightness-110 transition-all"
              >
                Sign in with Google
              </button>
            </motion.div>
              ...
              <main className="flex-1">
                ...
                >
                  <div className="max-w-2xl mx-auto">
                    {/* Page label */}
                    <div className="mb-8">
                      <div className="font-mono text-[10px] uppercase tracking-widest opacity-40 mb-1">02 / Transfer</div>
                      <h1 className="font-display italic font-black text-4xl tracking-tighter">Send Money</h1>
                    </div>

                    <div className="border border-[var(--border)]">
                      {/* Form header */}
                      <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)]" style={{background:'color-mix(in srgb, var(--accent) 4%, var(--background))'}}>  
                        <div className="flex items-center gap-3">
                          <button className="opacity-50 hover:opacity-100 transition-opacity">
                            <ArrowLeft className="h-4 w-4" />
                          </button>
                          <span className="font-mono text-[11px] uppercase tracking-widest" style={{color:'var(--accent)'}}>Nigerian Bank Transfer</span>
                        </div>
                      ...
                      <div className="p-6 space-y-6">
                        ...
                        <motion.div
                          ...
                          className="space-y-2"
                        >
                          <div className="font-mono text-[10px] uppercase tracking-widest opacity-40 mb-2 flex items-center gap-1"><Building2 className="h-3 w-3" /> Recipient's Bank</div>
                          <select
                            ...
                            className="w-full bg-white/5 border border-[var(--border)] px-3 py-2.5 font-mono text-xs focus:outline-none focus:border-[var(--accent)] transition-colors [&>option]:bg-gray-900"
                          >
                        ...
                        <motion.div
                          ...
                          className="space-y-2"
                        >
                          <div className="font-mono text-[10px] uppercase tracking-widest opacity-40 mb-2">Account Number</div>
                          <div className="flex gap-2">
                            <input
                              ...
                              className="flex-1 bg-white/5 border border-[var(--border)] px-3 py-2.5 font-mono text-xs tracking-widest focus:outline-none focus:border-[var(--accent)] transition-colors placeholder:opacity-30"
                            />
                            <button
                              onClick={handleVerifyAccount}
                              disabled={accountVerifying || recipientAccountNumber.length !== 10 || !recipientBankCode}
                              className="px-5 py-2.5 font-mono text-[10px] uppercase tracking-widest text-white transition-all disabled:opacity-30 hover:brightness-110"
                              style={{backgroundColor:'var(--accent)'}}
                            >
                              {accountVerifying ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Verify"}
                            </button>
                          </div>
                          ...
                          <AnimatePresence mode="wait">
                            {resolvedAccountName && (
                              <motion.div
                                ...
                                animate={{ opacity: 1, y: 0 }}
                                exit={{ opacity: 0 }}
                                className="flex items-center gap-2 px-3 py-2 border border-[var(--border)] bg-white/[0.03]"
                                style={{borderLeftColor:'var(--accent)', borderLeftWidth:'2px'}}
                              >
                                <User className="h-3.5 w-3.5 shrink-0" style={{color:'var(--accent)'}} />
                                <span className="font-mono text-xs" style={{color:'var(--accent)'}}>{resolvedAccountName}</span>
                              </motion.div>
                            )}
                          </AnimatePresence>

                          {/* Fraud / valid / invalid status */}
                          <AnimatePresence mode="wait">
                              ...
                              <motion.div
                                ...
                                className={`px-4 py-3 border-l-2 ${
                                  verificationStatus === "valid"
                                    ? "border-[var(--accent)] bg-white/[0.02]"
                              ...
                              >
                                {verificationStatus === "valid" && (
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="font-mono text-[11px] uppercase tracking-widest" style={{color:'var(--accent)'}}>Account verified — safe to proceed</span>
                                  </div>
                                )}
                                {verificationStatus === "fraud" && (
                                  <div className="space-y-2">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span className="font-mono text-[11px] uppercase tracking-widest text-red-400">Fraud risk detected — do not proceed</span>
                                    </div>
                                    <button
                                      onClick={handleSeeWhy}
                                      className="font-mono text-[10px] uppercase tracking-widest text-red-400 opacity-70 hover:opacity-100 flex items-center gap-1 transition-opacity"
                                    >
                                      See Why <ChevronRight className="h-3 w-3" />
                                    </button>
                                  </div>
                                )}
                                {verificationStatus === "invalid" && (
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="font-mono text-[11px] uppercase tracking-widest text-red-400">Account not found — check the details</span>
                                  </div>
                                )}
                              </motion.div>
                            )}
      ...
      <AlertDialog open={isAlertOpen} onOpenChange={setIsAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Why is this account flagged as fraudulent?</AlertDialogTitle>
            <AlertDialogDescription>
              This account number has been associated with multiple reported scams and fraudulent activities. It is strongly recommended not to proceed with any transactions to this account.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogAction>Understood</AlertDialogAction>
                        ...
                        <motion.div
                          ...
                          className="space-y-2"
                        >
                          <div className="font-mono text-[10px] uppercase tracking-widest opacity-40 mb-2">Amount</div>
                          <div className="relative">
                            <span className="absolute left-3 top-1/2 -translate-y-1/2 font-mono text-sm opacity-50">₦</span>
                            <input
                              type="number"
                              value={amount}
                              onChange={(e) => setAmount(e.target.value)}
                              placeholder="0"
                              className="w-full pl-8 pr-4 py-2.5 bg-white/5 border border-[var(--border)] font-mono text-sm focus:outline-none focus:border-[var(--accent)] transition-colors"
                            />
                          </div>
                        </motion.div>
                        ...
                        <motion.div
                          ...
                          className="space-y-3"
                        >
                          <div className="font-mono text-[10px] uppercase tracking-widest opacity-40 mb-2">Remarks (Optional)</div>
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                            {remarkOptions.map((option, index) => (
                              ...
                              >
                                <button
                                  className={cn(
                                    "h-10 w-full border font-mono text-[10px] uppercase tracking-widest transition-all",
                                    remarks === option.value
                                      ? "text-white border-[var(--accent)]"
                                      : "border-[var(--border)] opacity-50 hover:opacity-100 hover:border-[var(--foreground)]"
                                  )}
                                  style={remarks === option.value ? {backgroundColor:'var(--accent)'} : {}}
                      ...
                      <div className="border-t border-[var(--border)]">
                        <button
                          ...
                          disabled={!accountVerified || verificationStatus === 'fraud'}
                          className="w-full py-5 font-mono text-xs uppercase tracking-widest text-white transition-all disabled:opacity-30 disabled:cursor-not-allowed hover:brightness-110"
                          style={{backgroundColor:'var(--accent)'}}
                        >
                          Send Money
                        </button>
            ...
            {showPopup && (
              ...
              >
                <motion.div
                  ...
                  className="border border-[var(--border)] max-w-md w-full m-4 overflow-hidden"
                style={{background:'var(--background)'}}
                >
                  <div className="flex justify-between items-center px-6 py-4 border-b border-[var(--border)]">
                    <div className="font-mono text-[10px] uppercase tracking-widest opacity-40">Transaction Details</div>
                    <button
                    ...
                    {transactionData.length > 0 ? (
                      <ul className="divide-y divide-[var(--border)]">
                        {transactionData.map(([key, value], index) => (
                          ...
                          >
                            <span className="font-mono text-[10px] uppercase tracking-widest opacity-40">{key}</span>
                            <span className="font-mono text-xs">{value}</span>
                          </motion.li>
                        ))}
                      </ul>
                    ) : (
                      <p className="font-mono text-[11px] opacity-40">No data found</p>
                    )}
                  </div>
                  <div className="border-t border-[var(--border)]">
                    <button
                      onClick={() => setShowPopup(false)}
                      className="w-full py-4 font-mono text-xs uppercase tracking-widest text-white hover:brightness-110 transition-all"
                      style={{backgroundColor:'var(--accent)'}}
                    >
                      Close
                    </button>
