...
const generateUPIId = (name) => {
  ...
  return `${baseName}${randomSuffix}@shieldpay`;
};

const PIPELINE = [
  { tag: "[INPUT]",    title: "Raw Data Stream",  body: "20+ behavioral signals ingested per transaction — device, geo, velocity, biometrics." },
  { tag: "[PHASE 01]", title: "GAN Synthesis",    body: "Generative adversarial networks simulate future fraud vectors to harden the classifier." },
  { tag: "[PHASE 02]", title: "Random Forest",    body: "High-confidence classification through distributed, weighted decision ensembles." },
  { tag: "[OUTPUT]",   title: "Binary Verdict",   body: "Sub-40ms allow / challenge / block payload with full interpretability logs." },
];
...
const PRICING = [
  ...
  },
  {
    ...
    features: ["GAN-augmented training", "20+ behavioral signals", "Custom rule overlays", "SLA-backed uptime"],
    cta: "Quick Demo",
    featured: true,
  },
  {
    ...
    features: ["Dedicated model instance", "On-prem or VPC deploy", "24/7 risk desk", "Compliance & audit exports"],
    cta: "Contact sales",
    featured: false,
  },
];
    ...
    <main className="min-h-screen bg-background text-foreground font-sans selection:bg-accent selection:text-white">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 relative">
          <div className="lg:col-span-8 animate-reveal">
            ...
            <p className="max-w-[46ch] text-xl md:text-2xl text-muted font-display italic leading-relaxed text-pretty">
              Real-time adversarial defense powered by GAN-driven behavioral synthesis. We detect what others don't even see yet.
            </p>
          ...
          <div className="lg:col-span-4 flex flex-col justify-end gap-4 animate-reveal [animation-delay:150ms]">
            <div className="p-6 border border-border bg-white/5 backdrop-blur-sm relative overflow-hidden">
              ...
              <div className="font-mono text-[10px] uppercase opacity-60 leading-tight">Avg latency · real-time analysis</div>
            </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 divide-x divide-border">
          {METRICS.map((m, i) => (
            <div key={m.k} className="p-8">
              <p className="text-[10px] font-mono opacity-40 mb-3 uppercase tracking-widest">({String(i + 1).padStart(2, "0")}) {m.k}</p>
      ...
      <section id="pipeline" className="py-24 md:py-32 px-6 md:px-10 border-b border-border">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-4 mb-16">
            ...
            <h2 className="text-sm uppercase tracking-[0.3em] font-bold">The Processing Pipeline</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-px bg-border border border-border">
            {PIPELINE.map((p, i) => (
              <div
                key={p.tag}
                className={`bg-background p-8 min-h-[240px] flex flex-col justify-between ${i === PIPELINE.length - 1 ? "bg-accent/[0.04]" : ""}`}
              >
      ...
      <section id="dashboard" className="py-24 md:py-32 px-6 md:px-10 bg-white/[0.02] border-b border-border">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <div>
            <span className="font-mono text-[10px] uppercase tracking-widest text-accent mb-6 block">02 / Interface</span>
            <h2 className="text-5xl md:text-6xl font-display font-black italic tracking-tighter mb-8">Signals of intention.</h2>
            <p className="text-muted mb-10 max-w-[48ch] leading-relaxed">
              Every verdict is inspectable. Trace which of the twenty signals drove the outcome — not a black box, a reasoned defense.
            </p>
              ...
              {[
                ...
                <div
                  ...
                  className={`flex items-center gap-4 border-l-2 pl-6 py-3 transition-colors ${
                    s.active ? "border-accent" : "border-border opacity-50 hover:opacity-100 hover:border-foreground/40"
                  }`}
                >
          ...
          <div className="relative">
            ...
            <div className="absolute -bottom-6 -right-4 md:-bottom-10 md:-right-10 w-64 p-6 bg-accent text-white font-mono shadow-2xl shadow-accent/30">
              <div className="text-[10px] uppercase mb-4 opacity-80">Detected · anomaly #882</div>
              <div className="text-2xl font-black leading-none tracking-tighter">FRAUD STOPPED</div>
              <div className="mt-4 h-1 w-full bg-white/20 overflow-hidden">
                <div className="h-full w-[86%] bg-white" />
              </div>
              <div className="text-[10px] uppercase mt-2 opacity-70">confidence 0.862</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="signals" className="py-24 md:py-32 px-6 md:px-10 border-b border-border">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-4 mb-16">
            ...
            <h2 className="text-sm uppercase tracking-[0.3em] font-bold">Twenty Signals, One Verdict</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-border border border-border">
            {SIGNALS.map((s) => (
              <div key={s.n} className="bg-background p-6 hover:bg-white/[0.03] transition-colors group">
      ...
      <section id="pricing" className="py-24 md:py-32 px-6 md:px-10 border-b border-border">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-4 mb-16">
            ...
            <h2 className="text-sm uppercase tracking-[0.3em] font-bold">Pricing</h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-border border border-border">
            {PRICING.map((p) => (
              <div key={p.tier} className={`p-8 flex flex-col ${p.featured ? "bg-accent text-white" : "bg-background"}`}>
                ...
                <h4 className="text-2xl font-display italic mb-6">{p.tier}</h4>
                <div className="flex items-baseline gap-1 mb-2">
                  <span className="text-5xl font-mono tracking-tighter">{p.price}</span>
                  <span className={`text-xs font-mono ${p.featured ? "opacity-80" : "opacity-50"}`}>{p.per}</span>
                </div>
                <p className={`text-xs font-mono mb-8 ${p.featured ? "opacity-80" : "opacity-50"}`}>{p.note}</p>
                ...
                <button
                  onClick={p.featured ? handleGetStarted : undefined}
      ...
      <footer className="py-12 px-6 md:px-10 border-t border-border">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start gap-8">
          <div className="font-mono text-[10px] tracking-widest uppercase opacity-40">© 2026 Shieldpay Corp. All rights reserved.</div>
